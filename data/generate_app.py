...
function rselHTML(selId,vals,cur){
  return vals.map(v=>`<button class="rsel-btn${cur===v?' on':''}" onclick="shSelect('${selId}',${JSON.stringify(v)},this)">${v}</button>`).join('');
}
function shSelect(selId,val,btn){
  if(selId==='rarity') shRarity=val; else shPoints=val;
  btn.closest('.rsel').querySelectorAll('.rsel-btn').forEach(b=>b.classList.remove('on'));
